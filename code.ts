...
function evaluateTypographyHarmony(node: SceneNode, pattern: TypographyPattern): number {
  // Only apply to text nodes
  if (node.type !== 'TEXT') return 50;
...
function evaluateShapeHarmony(node: SceneNode, pattern: ShapePattern): number {
...
function generateInsights(node: SceneNode, pattern: StylePattern, scores: any): string[] {
  ...
  // Shape insights
  if ('cornerRadius' in (node as any) && scores.shapeScore < 70) {
...
function generatePlaceholderHarmonyScore(node: SceneNode): any {
...
figma.ui.onmessage = async (msg: any) => {
  ...
  if (msg.type === "run-scan") {
    ...
    if (currentMode === "compliance") {
      ...
      if (selectedComponentTypeFilter) {
        referenceNodes = referenceNodes.filter(node => 
          detectComponentType(node) === selectedComponentTypeFilter
        );
      }
      ...
      referenceNodes.forEach(node => {
        if (!isNodeTrulyVisible(node)) return;
